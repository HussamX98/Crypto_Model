...
class SpikeModelTrainer:
    ...
    def prepare_training_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in full_df.columns:
            if col not in ['token_address', 'spike_time', 'is_spike']:
                ...
                if len(spike_vals) > 0 and len(non_spike_vals) > 0:
                    self.logger.info(f"\n{col}:")
                    self.logger.info(f"  Spike - mean: {spike_vals.mean():.4f}, std: {spike_vals.std():.4f}")
                    self.logger.info(f"  Non-spike - mean: {non_spike_vals.mean():.4f}, std: {non_spike_vals.std():.4f}")
                    self.logger.info(f"  Difference ratio: {(spike_vals.mean() / non_spike_vals.mean()):.4f}")
    ...
    def save_feature_test_results(self, feature_scores: Dict, run_dir: str):
        ...
        with open(os.path.join(results_dir, 'feature_scores.json'), 'w') as f:
            json.dump(feature_scores, f, indent=4, cls=CustomJSONEncoder)
    ...
    def train_and_evaluate(self, df: pd.DataFrame) -> Dict:
        ...
        # Prepare training data (this now includes the is_spike column)
        training_df = self.prepare_training_data(df)
        ...
        # Analyze features including the target
        feature_analysis = self.analyze_features(training_df)  # Pass full training_df instead of just features
    ...
    def evaluate_holding_period(self, df: pd.DataFrame, holding_period: int) -> List[TradeResult]:
        """Evaluate trades with realistic returns based on holding period"""
        trades = []
        
        for _, entry in df.iterrows():
            try:
                entry_time = pd.to_datetime(entry['spike_time'])
                entry_price = float(entry['initial_price'])
                
                # Calculate realistic exit price based on holding period
                completion_ratio = min(holding_period / entry['spike_duration'], 1.0)
                potential_return = entry['future_price_increase'] - 1  # Convert multiple to return
                actual_return = potential_return * completion_ratio
                
                exit_price = entry_price * (1 + actual_return)
                
                # Calculate realistic max price during holding period
                max_completion = min((holding_period * 1.5) / entry['spike_duration'], 1.0)
                max_return = potential_return * max_completion
                max_price = entry_price * (1 + max_return)
                ...
                trades.append(TradeResult(
                    ...
                    feature_values={k: float(entry[k]) for k in self.feature_thresholds.keys()}
                ))
                
            except Exception as e:
                self.logger.warning(f"Error evaluating trade: {str(e)}")
                continue
    ...
    def _calculate_max_drawdown(self, returns: List[float]) -> float:
        ...
        try:
            ...
            self.logger.warning(f"Error calculating max drawdown: {str(e)}")
            return 0.0
